...
def analyze_logic(lat: float, lon: float, beach_dir: str, w_data: Dict, m_data: Dict) -> Dict[str, Any]:
    ...
    if score >= 35 and sw == "None" and wr in ["None", "Low"] and rip in ["Low"]:
        verdict = "ممتاز"
        explanation = "ظروف ممتازة: موج مناسب (0.5-1.2م)، رياح خفيفة، وانخفاض ضغط بطيء ينشط الأسماك."
    elif score >= 20 and "Confirmed" not in [sw, wr]:
        verdict = "ممكن"
        explanation = "ظروف مقبولة للصيد مع بعض التحديات. استخدم معدات أثقل قليلاً."
    elif score >= 5:
        verdict = "صعب جداً"
        explanation = "ظروف قاسية: أمواج عاتية أو رياح شديدة أو طحالب متراكمة."
    else:
        verdict = "مستحيل"
    ...
    return {
        ...
        "verdict": {"status": verdict, "score": score, "explanation": explanation, "sinker_weight_g": sinker}
    }
