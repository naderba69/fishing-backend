...
def analyze_logic(lat: float, lon: float, beach_dir: str, w_data: Dict, m_data: Dict) -> Dict[str, Any]:
    ...
    wd = safe_get(w_h.get("wind_direction_10m", []), idx, 270.0)    p_now = safe_get(w_h.get("surface_pressure", []), idx, 1015.0)
...
@app.post("/best-spots")async def best_spots(custom: List[SpotReq]):
